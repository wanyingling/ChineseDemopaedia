        ...
        if element.name == 'h1':
            ...
            if notes:
                if current_definition:
                    current_definition += "<br>备注：" + notes  # 使用 HTML 换行符 <br>
                else:
                    current_definition = "备注：" + notes
...
def write_to_csv(data, filename):
    cc = OpenCC('s2hk')  # Convert from Simplified Chinese to Traditional Chinese (Hong Kong)
    ...
    with open(filename, 'w', newline='', encoding='utf-8-sig') as file:
        ...
        for row in data:
            traditional_definition = cc.convert(row[3])  # Convert the definition to Traditional Chinese (HK) while preserving HTML tags
            traditional_section = cc.convert(row[1])  # Convert the section to Traditional Chinese (HK)
            new_row = row[:4] + [traditional_section] + [traditional_definition]
            writer.writerow(new_row)


# Read the HTML content from a file
with open('Demopedia_cn_simple.html', 'r', encoding='utf-8') as file:
...
# Write to CSV
write_to_csv(extracted_data, 'demopedia_cn_trad.csv')
